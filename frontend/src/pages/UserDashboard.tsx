  ...
  return (
    <div className="max-w-[1100px] flex flex-col gap-9 pb-16">
      ...
      <div className="flex flex-wrap items-end justify-between gap-6">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[14px] text-low">
            {isPremium ? 'Licencia vitalicia activa: análisis y descargas sin límite.' : `Te quedan ${availableToday} análisis gratuitos hoy.`}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 border-y hair">
        ...
        <Stat label="Documentos"><CountUp value={totalAnalysesCount} /></Stat>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-6">
        ...
        <div className="card p-7 flex flex-col gap-7">
          ...
          <div className="flex flex-col gap-3 pt-5 border-t hair">
            ...
            <div className="flex h-[6px] rounded-full overflow-hidden bg-hair">
              <div className="bg-human transition-all duration-900 ease-out" style={{ width: `${pct(highAuthenticity)}%` }} />
              <div className="bg-mixed transition-all duration-900 ease-out" style={{ width: `${pct(moderateAi)}%` }} />
              <div className="bg-ai transition-all duration-900 ease-out" style={{ width: `${pct(highAi)}%` }} />
            </div>
      ...
      {!isPremium && (
        <div className="relative overflow-hidden rounded-[18px] border border-azure/30 p-7 sm:p-8 flex flex-wrap items-center justify-between gap-6"
          ...
          <button type="button" onClick={() => { sound.playClick(); openPremiumModal(); }} className="btn btn-primary shrink-0"><Crown className="w-4 h-4" strokeWidth={1.8} /> Activar por $2</button>
        </div>
      )}
      {isPremium && (
      ...
      <div className="flex flex-col gap-5">
        ...
        {loadingHistory ? (
          <div className="flex items-center gap-3 py-10 text-[13px] text-mid"><Loader2 className="w-4 h-4 animate-spin text-azure" /> Cargando…</div>
        ) : recent.length === 0 ? (
          ...
          <div className="flex flex-col">
            {recent.map((item, i) => {
              ...
              return (
                <button key={item.id} type="button" onClick={() => { sound.playClick(); navigate(`/analyzer?id=${item.id}`); }}
                  className={`group flex items-center gap-5 py-4 border-b hair text-left hover:bg-hair transition-colors duration-450 ${i === 0 ? 'border-t' : ''}`}>
                  ...
                  <ArrowRight className="w-4 h-4 text-low opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" strokeWidth={1.8} />
                </button>
              );
            })}
          </div>
